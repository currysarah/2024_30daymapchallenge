...
d05_work <- d05_work %>%
  mutate(linecolor = case_when(distmile <= 60 ~ "Less than 1 Hour",
                               distmile > 60 ~ "Over 1 Hour"))

d05_map <- d05_work %>%
  filter(distmile <= 100 & distmile > 0)

ggplot() + 
  geom_histogram(data = d05_map %>% filter(Workers.in.Commuting.Flow <= 10000), aes(x = Workers.in.Commuting.Flow), fill = "darkolivegreen4", color = "darkolivegreen", bins = 60)

quantile(d05_map$Workers.in.Commuting.Flow)

d05_map <- d05_map %>%
  mutate(percentile = case_when(Workers.in.Commuting.Flow <= 72 ~ "u25",
                                Workers.in.Commuting.Flow <= 180 ~ "u50",
                                Workers.in.Commuting.Flow <= 571 ~ "u75",
                                Workers.in.Commuting.Flow > 571 ~ "u100"))
...
ggplot() + 
#  geom_sf(data = map_states) +
#, fill = "gray", color = "white", size = 4) +
  geom_segment(data = d05_map %>%
                 filter(percentile == "u25"), aes(x = rlat, y = rlon, xend = wlat, yend = wlon), color = "#E5866C", size = 0.25, alpha = 0.25, curvature = 0.0) +
  geom_segment(data = d05_map %>%
                 filter(percentile == "u50"), aes(x = rlat, y = rlon, xend = wlat, yend = wlon), color = "#8F5443", size = 0.25, alpha = 0.50, curvature = 0.0) +
  geom_segment(data = d05_map %>%
                 filter(percentile == "u75"), aes(x = rlat, y = rlon, xend = wlat, yend = wlon), color = "#435DBA", size = 0.25, alpha = 0.75, curvature = 0.0) +
  geom_segment(data = d05_map %>%
                 filter(percentile == "u100"), aes(x = rlat, y = rlon, xend = wlat, yend = wlon), color = "#2E4080", size = 0.25, alpha = 1, curvature = 0.0) +
#  scale_color_gradient2(limits = c(0, 50000), low = "lightcyan", high = "royalblue4", transform = "log1p") +
  ...
  labs(title = "Commute Patterns Between Counties Pre-Covid",
       subtitle = "Can bus/carpool/train routes be better informed by tracking how many people typically commute between counties?",
       caption = "Data Source: US Census Bureau, ACS 5-year, 2016-2020") +
  theme(plot.title = element_textbox_simple(size = 14, lineheight = 1, halign = 0.0, padding = margin(10, 10, 5, 30)),
        plot.subtitle = element_textbox_simple(size = 9, lineheight = 1, halign = 0.0, padding = margin(5, 10, 20, 30)),
        plot.caption = element_textbox_simple(size = 6, lineheight = 1, halign = 1),
...
ggsave(filename = "thumbnails/05_scurry_journey.png", width = 6, height = 6, dpi = 300)
