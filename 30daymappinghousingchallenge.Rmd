...
```{r day25_data, message=FALSE, warning=FALSE}

d25_data <- read.csv(file = "data/d25_source.csv")
...
```{r day25_work, message=FALSE, warning=FALSE}
...
```{r day25_map, message=FALSE, warning=FALSE, fig.width=6, fig.height=6}
...
ggplot() + 
    geom_sf(data = map_states, fill = "gray85", color = "white") +  # Add Chicago boundary
    geom_sf(data = d25_work, aes(color = yr_pis), size = 0.1) +  # Overlay burglaries
    labs(title = "LIHTC Projects, 1990 - 2020") +  # Set plot title
  xlim(-2150000, 2074000) +
  ylim(400000.00, 3500000) +
    theme_void() +
  theme(legend.position = "bottom") # Use a blank theme
  
  # Plot 2: Density of burglaries with contours overlaid on Chicago boundary
ggplot() + 
    geom_sf(data = map_states, fill = "gray85", color = "white") +  # Add Chicago boundary with grey fill
    stat_density2d(data = data.frame(st_coordinates(d25_work)),  # Compute 2D kernel density estimate
                   aes(X, Y, fill = ..level.., alpha = ..level..),  # Define aesthetics for density contours
                   size = 0.01, bins = 80, geom = 'polygon') +  # Set size and number of bins for contours
    ...
    labs(title = "Density of LIHTC Projects") +  # Set plot title
  xlim(-2150000, 2074000) +
  ylim(400000.00, 3500000) +
